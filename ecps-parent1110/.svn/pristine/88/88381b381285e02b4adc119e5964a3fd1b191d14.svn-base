...
function modify(shipAddrId){
	$.ajax({
		...
		success:function(responseText){
			//{"shipAddr":{"addr":"gfnan","city":"gz","defaultAddr":1,
			//"district":"gnsf","fixedPhone":"","phone":"18370876897","province":"jx",
			//"ptlUserId":1001,"shipAddrId":100054,"shipName":"sihai","zipCode":"341000"}}
			var shipAddr = $.parseJSON(responseText);
			$("#shipName").val(shipAddr.shipAddr.shipName);
			$("#addr").val(shipAddr.shipAddr.addr);
			$("#zipCode").val(shipAddr.shipAddr.zipCode);
			$("#phone").val(shipAddr.shipAddr.phone);
			$("#defaultAddr").removeAttr("checked");
			$("#shipAddrId").val(shipAddr.shipAddr.shipAddrId);
			if(shipAddr.shipAddr.defaultAddr == "1"){
		...
		<div class="confirm">
			...
			<div class="ofc">
				...
				<c:forEach items="${addrList }" var="addr">
					...
					<td>${addr.zipCode }</td>
					<td>${addr.phone }</td>
					<td class="def"><a href="${path }/user/login/updateDefault.do?shipAddrId=${addr.shipAddrId}" title="设为默认">设为默认</a></td>
					...
					<input type="hidden" id="listCount" value="${fn:length(addrList)}">
				</c:forEach>
				</tbody>
				</table>
				...
				<form id="jvForm" action="${path }/user/login/saveOrUpdateShipAddr.do" method="post">
					<input type="hidden" id="shipAddrId" name="shipAddrId" value=""/>
					<ul class="uls form">
